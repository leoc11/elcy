...
export class SelectManyQueryable<S, T> extends Queryable<T> {
    protected get selector() {
        if (!this._selector && this.selectorFn) {
            this._selector = ExpressionBuilder.parse<IEnumerable<T>>(this.selectorFn, [this.parent.type], this.parameters);
        }
    ...
    constructor(public readonly parent: Queryable<S>, selector: FunctionExpression<IEnumerable<T>> | ((item: S) => IEnumerable<T>), public type: GenericType<T> = Object) {
        super(type, parent);
    ...
    protected _selector: FunctionExpression<IEnumerable<T>>;
