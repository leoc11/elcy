...
export function SerializeColumn<TE extends object, T extends object>(type: IObjectType<T>, name?: string, defaultValue?: () => T): ClassPropertyDecorator<TE, T>;
export function SerializeColumn<TE extends object, T extends object>(optionOrType: IObjectType<T> | ISerializeColumnOption<T>, name?: string, defaultValue?: () => T): ClassPropertyDecorator<TE, T> {
    ...
    if (optionOrType && typeof optionOrType !== "function") {
        ...
        if (typeof optionOrType !== "undefined") {
            option.type = optionOrType as IObjectType<T>;
        }
