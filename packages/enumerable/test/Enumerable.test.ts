...
describe("ENUMERABLE", () => {
    ...
    const items2 = Enumerable.from([[1, 2], [3, 4]]);
    const objArray = Enumerable.from([{ position: 6, value: 6 }, { position: 3, value: 12 }, { position: 1, value: 1 }, { position: 3, value: 1 }]);
    describe("DISTINCT", () => {
        it("should work", () => {
    ...
    describe("CROSSJOIN", () => {
        it("should work", () => {
            const distincts = Enumerable.from([1, 2, 3, 4, 5]).crossJoin([1, 5], (o1, o2) => o1 + o2);
            let index1 = 0;
            for (const { } of distincts) {
                index1++;
            }
