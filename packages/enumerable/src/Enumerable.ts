...
declare module "./Enumerable" {
    interface Enumerable<T> {
        ...
        pivot<TD extends { [key: string]: (item: T) => unknown }, TM extends { [key: string]: (item: T[]) => unknown }>(dimensions: TD, metrics: TM): Enumerable<Pivot<T, TD, TM>>;
        ...
        flatMap<TReturn>(selector: (item: T) => Iterable<TReturn>): Enumerable<TReturn>;
...
Enumerable.prototype.map = function <T, TReturn>(this: Enumerable<T>, selector: (item: T) => TReturn): Enumerable<TReturn> {
    return new SelectEnumerable(this, selector);
};
Enumerable.prototype.flatMap = function <T, TReturn>(this: Enumerable<T>, selector: (item: T) => TReturn[] | Enumerable<TReturn>): Enumerable<TReturn> {
    return new SelectManyEnumerable(this, selector);
};
Enumerable.prototype.filter = function <T>(this: Enumerable<T>, predicate: (item: T) => boolean): Enumerable<T> {
...
Enumerable.prototype.slice = function <T>(this: Enumerable<T>, skip: number, take: number): Enumerable<T> {
...
Enumerable.prototype.union = function <T>(this: Enumerable<T>, array2: IEnumerable<T>): Enumerable<T> {
    return new UnionEnumerable(this, Enumerable.from(array2));
};
Enumerable.prototype.concat = function <T>(this: Enumerable<T>, ...items: Iterable<T>[]): Enumerable<T> {
    // eslint-disable-next-line @typescript-eslint/no-this-alias
    let result: Enumerable<T> = this;
...
Enumerable.prototype.pivot = function <T, TD extends { [key: string]: (item: T) => unknown }, TM extends { [key: string]: (item: T[]) => unknown }>(this: Enumerable<T>, dimensions: TD, metrics: TM): Enumerable<Pivot<T, TD, TM>> {
...
export const keyComparer = <T = unknown>(a: T, b: T) => {
    ...
    if (!result && isNotNull(a) && isNotNull(b) && a instanceof Object && b instanceof Object) {
        ...
        if (result) {
            result = Enumerable.from(aKeys).every((o) => Object.prototype.hasOwnProperty.call(b, o) && b[o] === a[o]);
        }
    }
    return result;
};
