...
export class UpsertDeferredQuery<T> extends DMLDeferredQuery<T> {
    ...
    protected buildQueries(visitor: IQueryVisitor): Array<QueryExpression<T[]>> {
        ...
        const paramEntity = new MemberAccessExpression(new ParameterExpression<EntityEntry<T>>("entry", EntityEntry), "entity", this.entry.metaData.type);
        const value: { [K in keyof T]?: IExpression } = {};
        for (const prop of this.entry.metaData.primaryKeys.select((o) => o.propertyName).union(this.insertProperties)) {
            const relationIdValue = new MemberAccessExpression(new ParameterExpression("relationId", Object), prop);
            const entityValue = new MemberAccessExpression(paramEntity, prop);
            const relationIdNotExist = new EqualExpression(relationIdValue, new ValueExpression(null));
            const existingParam = value[prop];
            if (existingParam instanceof SqlParameterExpression) {
                queryExp.parameterTree.node.delete(existingParam);
            }
            value[prop] = queryExp.addSqlParameter(visitor.newAlias("param"), new TernaryExpression(relationIdNotExist, entityValue, relationIdValue));
        }

        if (this.entry.metaData.concurrencyMode === "PESSIMISTIC") {
            // TODO
        }

        let modifiedProperties = this.entry.getModifiedProperties();
        if (!modifiedProperties.any()) {
            modifiedProperties = this.entry.metaData.columns.where((o) => !o.isPrimaryColumn)
                .select((o) => o.propertyName).toArray();
        }

        const upsertExp = new UpsertExpression(queryExp.entity as EntityExpression, value, modifiedProperties);
        upsertExp.parameterTree = queryExp.parameterTree;
        results.push(upsertExp);
