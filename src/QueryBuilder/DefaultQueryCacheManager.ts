...
export class DefaultQueryCacheManager implements IQueryCacheManager {
    public async get(type: IObjectType<DbContext>, key: string) {
        const cached: Map<string, QueryCache> = Reflect.getOwnMetadata(queryCacheKey, type);
        if (cached && cached.has(key))
            return Promise.resolve(cached.get(key));
        return Promise.resolve(undefined);
    }
    public async set<T>(type: IObjectType<DbContext>, key: string, query: string, queryParser: IQueryResultParser<T>) {
        let cached: Map<string, QueryCache> = Reflect.getOwnMetadata(queryCacheKey, type);
        if (!cached) {
            cached = new Map<string, QueryCache>();
