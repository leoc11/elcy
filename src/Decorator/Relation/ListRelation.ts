...
export function ListRelation<S, T>(masterType: IObjectType<T> | IRelationOption<S, T>, sourceKeySelectors?: Array<(source: S) => any>, targetKeySelectors?: Array<(source: T) => any>, masterRelationPropertySelector?: (master: T) => S[], updateOption?: ReferenceOption, deleteOption?: ReferenceOption): PropertyDecorator {
    ...
    if (Object.keys(relationOption.relationMap).any((o) => !targetMetaData.primaryKeys.contains(o) && (targetUniqueKeys.length < 0 || targetUniqueKeys.all((key) => !targetMetaData.indices[key] || !targetMetaData.indices[key].properties.contains(o)))))
    ...
    return (target: S, propertyKey: string /* | symbol*//*, descriptor: PropertyDescriptor*/) => {
        ...
        if (!relationOption.name)
            relationOption.name = "FK_" + relationOption.slaveType!.name + "_" + relationOption.masterType.name;
        ...
        if (relationOption.masterRelationProperty) {
            const masterRelation = new MasterRelationMetaData(relationOption.masterType, relationOption.slaveType!, relationOption.name, RelationType.OneToMany, propertyKey);
